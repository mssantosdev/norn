...
func TestBasicWorkflow(t *testing.T) {
	...
	if err := cli.Run([]string{"tools", "add", "lint", "lint", "npm run lint"}); err != nil {
	...
	for _, path := range []string{
		filepath.Join(root, ".norn", "patterns", "api-contract.md"),
		filepath.Join(root, ".norn", "skills", "deploy-flow.md"),
		filepath.Join(root, ".norn", "tools", "lint.yaml"),
